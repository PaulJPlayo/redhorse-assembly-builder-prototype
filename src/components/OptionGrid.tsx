...
export const OptionGrid = ({ children, className, desktopCarousel = false }: OptionGridProps) => {
  ...
  const mobileClasses =
    "flex w-full min-w-0 max-w-full snap-x snap-mandatory gap-6 overflow-x-auto overscroll-x-contain px-1 pb-2 scroll-smooth [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden";

  const desktopClasses = desktopCarousel
    ? "sm:flex sm:flex-nowrap sm:gap-4 sm:overflow-x-auto sm:overscroll-x-contain sm:px-0 sm:pb-2 sm:snap-x sm:snap-mandatory sm:[-ms-overflow-style:none] sm:[scrollbar-width:none] sm:[&::-webkit-scrollbar]:hidden"
